    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">Загрузить аватар пользователя</h5>
...
export default defineComponent({
  ...
  methods: {
    showModal() {
      window.setTimeout(() => Modal.getOrCreateInstance('#editImageModal')?.show(), 50)
    },
    triggerFileInput() {
      if (this.fileInput) {
    ...
    async saveChanges() {
      if (!this.imageFile) {
        Modal.getInstance('#editImageModal')?.hide();
        return;
      }
